...
contract MorarableMarketContract {
    ...
    modifier HasTransferApproval(address tokenAddress,uint256 tokenId){
     ...
     modifier ItemExists(uint256 id){
        require(id < itemsForSale.length && itemsForSale[id].id==id, "Could Not Find Items");
        _;
    }
    ...
    function addItemToMaket(uint256 tokenId,address tokenAddress,uint256 askingPrice) OnlyItemOwner(tokenAddress,tokenId) HasTransferApproval(tokenAddress,tokenId) external returns (uint256){
